...
class SendMail:
    ...
    def sendMail(self,recipients,reply_to,subject,body):            
        ...
        rcptto = recipients
        # 显示的Cc收信地址
        # rcptcc = ['kzhou2017@outlook.com']
        # Bcc收信地址，密送人不会显示在邮件上，但可以收到邮件
        rcptbcc =  ['kzhou2017@outlook.com']
        # 全部收信地址，包含抄送地址，单次发送不能超过60人
        receivers = rcptto  + rcptbcc

        # 构建alternative结构
        msg = MIMEMultipart('alternative')
        msg['Subject'] = Header(subject)
        msg['From'] = formataddr(["Nightwish Tech", username])  # 昵称+发信地址(或代发)
        # list转为字符串
        msg['To'] = ",".join(rcptto)
        # msg['Cc'] = ",".join(rcptcc)
        ...
        try:
            ...
            # print(receivers)
            res=client.sendmail(username, receivers, msg.as_string())  # 支持多个收件人，具体数量参考规格清单
            client.quit()
            logger.info('email sent successfully!')
            return True
        except smtplib.SMTPConnectError as e:
            logger.error('email sent unsuccessfully:', e.smtp_code, e.smtp_error)
            return False
        except smtplib.SMTPAuthenticationError as e:
            logger.error('email sent unsuccessfully:', e.smtp_code, e.smtp_error)
            return False
        except smtplib.SMTPSenderRefused as e:
            logger.info('email sent unsuccessfully:', e.smtp_code, e.smtp_error)
            return False
            ...
            logger.error('email sent unsuccessfully: ', str(e))
            return False
        except Exception as e:
            logger.info('email sent unsuccessfully: ', str(e))
            return False